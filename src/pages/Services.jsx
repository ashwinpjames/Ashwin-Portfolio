...
function ServiceStack() {
  return <div className="service-stack" aria-label="Explore services">
    ...
    {allServices.map((service, index) => <Link key={service.id} to={`/services/${service.id}`} className={`service-stack-item service-stack-item-${index + 1}`} style={{ '--service-delay': `${index * 70}ms`, '--wave-delay': `${index * -320}ms` }}>
      <span className="service-stack-number">{service.number}</span>
