...
export default function BlogPromptArchitect() {
  ...
  const completion = useMemo(() => {
    ...
    const optional = [form.secondaryKeywords, form.searchIntent, form.audience, form.location, form.articleType, form.wordCount, form.tone, form.internalLinks, form.externalSources, form.extraRequirements]
      ...
      <div className="container prompt-architect-layout">
        <div className="prompt-architect-form-card">
          <div className="pa-topbar">
            <div>
              <span className="prompt-architect-eyebrow">CONTENT BRIEF</span>
              <h2>Tell the architect what you need.</h2>
            </div>
            <button className="pa-clear" type="button" onClick={clearForm}>Clear</button>
          </div>

          <div className="pa-progress">
            <div className="pa-progress-track"><span style={{ width: `${completion}%` }} /></div>
          ...
          <div className="pa-section">
            ...
            <div className="pa-two-col">
              <Field label="Target audience"><input className="pa-input" value={form.audience} onChange={(e) => update('audience', e.target.value)} placeholder="e.g. UAE SME founders" /></Field>
              <Field label="Target location"><input className="pa-input" value={form.location} onChange={(e) => update('location', e.target.value)} placeholder="e.g. UAE, India, Global" /></Field>
            </div>
            <div className="pa-two-col">
              <Field label="Target word count"><input className="pa-input" value={form.wordCount} onChange={(e) => update('wordCount', e.target.value)} placeholder="e.g. 1,800 to 2,200" /></Field>
              <Field label="Tone"><input className="pa-input" value={form.tone} onChange={(e) => update('tone', e.target.value)} placeholder="e.g. authoritative and conversational" /></Field>
            </div>
          </div>
          ...
          <div className="pa-advanced-toggle" onClick={() => setShowAdvanced((value) => !value)} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && setShowAdvanced((value) => !value)}>
          ...
          {showAdvanced && <div className="pa-advanced-panel">
            <Field label="Internal links to include" hint="Pages or URLs you want the final writer to consider. They will only be used where relevant."><textarea className="pa-input" value={form.internalLinks} onChange={(e) => update('internalLinks', e.target.value)} placeholder="https://ashwinjames.com/services/performance-marketing" rows={4} /></Field>
        ...
        <aside className="prompt-architect-output-card">
          <div className="pa-output-sticky">
            <div className="pa-output-head">
              <div><span className="prompt-architect-eyebrow">GENERATED PROMPT</span><h2>Your writing instructions.</h2></div>
              {prompt && <button className="prompt-architect-copy" type="button" onClick={copyPrompt}>{copied ? '✓ Copied' : 'Copy prompt'}</button>}
            </div>
