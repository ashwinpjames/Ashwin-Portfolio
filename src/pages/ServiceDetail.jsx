...
export default function ServiceDetail() {
  ...
  return <main className="service-detail-page" id="main">
    ...
    <section className="service-detail-section"><div className="container service-detail-columns">
      <div><p className="services-eyebrow">What it covers</p><h2>A focused service built around the outcome.</h2></div>
      <div>{points.length > 0 ? <ul className="service-detail-list">{points.map((point) => <li key={point}>{point}</li>)}</ul> : <p className="service-detail-copy">This service connects the right strategy, execution and measurement around the commercial problem it is designed to solve.</p>}<div className="service-detail-outcome"><span>{service.bestFor ? 'Best for' : 'Outcome'}</span><strong>{outcome}</strong></div></div>
    </div></section>
