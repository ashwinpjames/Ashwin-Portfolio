...
function renderMarkdown(markdown) {
  ...
  while (i < lines.length) {
    ...
    const paragraph = []
    while (i < lines.length && lines[i].trim() && !/^#{1,3}\s+/.test(lines[i]) && !/^[-*]\s+/.test(lines[i]) && !(lines[i].includes('|') && i + 1 < lines.length && /^\s*\|?\s*:?-+/.test(lines[i + 1]))) {
...
function FunnelGraphic() {
  ...
  return <div className="meta-sales-funnel" aria-label="Meta Ads to revenue funnel infographic">
    ...
    <div className="meta-sales-funnel-stages">
      {stages.map((stage, index) => <div className="meta-sales-funnel-stage" key={stage} style={{ '--stage': index }}><span>{String(index + 1).padStart(2, '0')}</span><strong>{stage}</strong></div>)}
    </div>
...
function ResponseGraphic() {
  return <div className="meta-sales-response" aria-label="Lead response time infographic">
    ...
    <div className="meta-sales-response-track">
      {['1 min', '5 min', '30 min', '6+ hrs'].map((time, index) => <div key={time} className={`meta-sales-response-step step-${index}`}><b>{time}</b><span>{index < 2 ? 'high priority' : index === 2 ? 'lower intent' : 'risk of loss'}</span></div>)}
    </div>
  </div>
}

function MetricsGraphic() {
  const metrics = [['CPL', 'Can acquire leads'], ['CPQL', 'Can acquire qualified leads'], ['CAC', 'Can acquire customers'], ['ROAS', 'Can generate revenue efficiency']]
  return <div className="meta-sales-metrics" aria-label="Performance marketing metrics infographic">
    ...
    <strong>Move from lead volume to business outcomes</strong>
  ...
  return <main className="meta-sales-blog-page">
    ...
    <article className="meta-sales-blog-shell">
      ...
      <div className="meta-sales-blog-content">
        <div dangerouslySetInnerHTML={{ __html: renderedArticle.split('<h2>2. Why Cheap Leads').join('<div class="meta-sales-visual-break"><CampaignComparison /></div><h2>2. Why Cheap Leads') }} />
      </div>

      <div className="meta-sales-inline-visual"><ResponseGraphic /></div>
      <div className="meta-sales-inline-visual"><MetricsGraphic /></div>
