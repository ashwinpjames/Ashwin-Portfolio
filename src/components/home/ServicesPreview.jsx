...
export default function ServicesPreview() {
  return <section id="services" className="home-section services-preview"><div className="container"><div className="section-heading split-heading reveal-home"><div><p className="home-eyebrow">Growth services</p><h2>The services UAE businesses ask for most.</h2></div><div><p>Start with the service that removes your biggest growth constraint. Each engagement is connected to <a href="https://www.linkedin.com/top-content/sales/identifying-high-value-leads/best-metrics-for-evaluating-lead-quality/" target="_blank" rel="noreferrer">lead quality</a>, conversion and commercial outcomes.</p><a className="text-link" href="/services">View all services <span>→</span></a></div></div><div className="services-grid six">{services.map(([title, text, outcome, anchor], index) => <a className="service-card reveal-home" href={`/services${anchor}`} key={title}><span className="service-number">0{index + 1}</span><div><h3>{title}</h3><p>{text}</p><strong>Outcome: {outcome} <span>→</span></strong></div><span className="service-arrow">↗</span></a>)}</div><div className="service-footer"><a className="home-button secondary" href="/services">See all services <span>→</span></a><p>Explore all 10 services, outcomes and engagement options.</p></div></div></section>
}
